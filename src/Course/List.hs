...
flatten Nil = Nil
flatten (xs :. Nil) = xs
flatten (xs :. ys :. rest) = flatten ((xs ++ ys) :. rest)
...
-- seqOptional (Full x :. xs) = Full $ x :. seqOptional xs

seqOptional = foldRight combine (Full Nil) where
  combine Empty _ = Empty
  combine (Full _) Empty = Empty
  combine (Full x) (Full memo) = Full (x :. memo)
...
lengthGT4 ::
  List a
  -> Bool
lengthGT4 xs = countGT4 xs 0 where
  countGT4 Nil counter = counter > 4
  countGT4 (_ :. ys) counter = if counter > 4 then True else countGT4 ys (counter + 1)
...
reverse ::
  List a
  -> List a
reverse = foldLeft (\memo x -> x :. memo) Nil
