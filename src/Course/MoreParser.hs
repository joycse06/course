...
noneof ::
  Chars
  -> Parser Char
noneof s = satisfy (\x -> notElem x s)
...
hex ::
  Parser Char
hex = --readHex $ replicateA 4 (satisfy isHexDigit)
...
betweenSepbyComma ::
  ...
  -> Parser a
  -> Parser (List a)
betweenSepbyComma x s pa =
  betweenCharTok x s $ pa `sepby` charTok ','
