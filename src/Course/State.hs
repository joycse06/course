...
instance Functor (State s) where
  (<$>) ::
    ...
    -> State s a
    -> State s b
  (<$>) f s = State (\s' -> let (r, s'') = runState s s' in (f r, s''))
...
findM f (h :. t) =
  (\q -> if q then pure (Full h) else findM f t) =<< f h
...
firstRepeat ::
  ...
  -> Optional a
firstRepeat list =
  eval (findM (State . lift2 (lift2 (,)) S.member S.insert) list) S.empty
